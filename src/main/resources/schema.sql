...
-- -----------------------------------------------------
-- Table `utopia`.`seat_type`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `utopia`.`seat_type` ;

CREATE TABLE IF NOT EXISTS `utopia`.`seat_type` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  PRIMARY KEY (`id`))
...
-- Table `utopia`.`seat`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `utopia`.`seat` ;

CREATE TABLE IF NOT EXISTS `utopia`.`seat` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `flight_id` INT UNSIGNED NOT NULL,
  `type_id` INT UNSIGNED NOT NULL,
  `price` FLOAT NOT NULL,
  `num_reserved` INT UNSIGNED NOT NULL,
  INDEX `fk_seat_type_id_idx` (`type_id` ASC) VISIBLE,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_seat_type_id`
    FOREIGN KEY (`type_id`)
    REFERENCES `utopia`.`seat_type` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `utopia`.`flight_bookings` (
  ...
  CONSTRAINT `fk_flight_bookings_seat`
    FOREIGN KEY (`seat_id`)
    REFERENCES `utopia`.`seat` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `utopia`.`flight_status` (`id` INT, `route_id` INT, `airplane_id` INT, `departure_time` INT, `arrival_time` INT, `max_capacity` INT, `passenger_count` INT, `reserved_seats` INT, `available_seats` INT);
...
CREATE  OR REPLACE VIEW `flight_status` AS SELECT
    ...
    flight_reserved_seats.reserved_seats,
    (airplane_capacity.max_capacity - flight_reserved_seats.reserved_seats - flight_passenger_count.passenger_count) as available_seats
    FROM
	flight
    INNER JOIN
    (SELECT
    ...
    (SELECT
		seat.flight_id,
        sum(seat.num_reserved) as reserved_seats
        FROM
        seat
        INNER JOIN
        flight
        ON flight.id=seat.flight_id
		GROUP BY flight_id
	) AS flight_reserved_seats
    ON flight.id=flight_reserved_seats.flight_id
    INNER JOIN
	(SELECT
		flight_id,
...
CREATE  OR REPLACE VIEW `flight_passengers` AS SELECT
	seat.flight_id, flight_bookings.booking_id,
    passenger.id as passenger_id
    FROM
    seat
    INNER JOIN
    flight_bookings
    ON seat.id=flight_bookings.seat_id
    INNER JOIN
    passenger
...
START TRANSACTION;
USE `utopia`;
INSERT INTO `utopia`.`flight` (`id`, `route_id`, `airplane_id`, `departure_time`, `arrival_time`) VALUES (1, 1, 1, '2021-01-28 12:00:00', NULL);
INSERT INTO `utopia`.`flight` (`id`, `route_id`, `airplane_id`, `departure_time`, `arrival_time`) VALUES (2, 2, 1, '2021-01-28 12:00:00', NULL);
...
START TRANSACTION;
USE `utopia`;
INSERT INTO `utopia`.`flight_bookings` (`seat_id`, `booking_id`) VALUES (1, 1);
INSERT INTO `utopia`.`flight_bookings` (`seat_id`, `booking_id`) VALUES (1, 2);
INSERT INTO `utopia`.`flight_bookings` (`seat_id`, `booking_id`) VALUES (2, 3);
INSERT INTO `utopia`.`flight_bookings` (`seat_id`, `booking_id`) VALUES (2, 5);
